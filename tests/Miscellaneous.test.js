...
const {Stringable, str} = require('../src/Stringable');
...
it('can flush cached words', function () {
    ...
    expect(Str._camelCache).not.toStrictEqual({});
...
it('returns Stringable instance using helper function `str`', () => {
    expect(str('Taylor'))
        .toBeInstanceOf(Stringable);

    expect(str().after('This is my name', 'This is').toString())
        .toBe(' my name');
});